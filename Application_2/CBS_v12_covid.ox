...
SetStsmModel1(const cy, const vpar, const fprint) // MODEL 1
{
	...
	if (columns(s_mY)>ct_covid) mxtrel[0][ct_covid-1] = 10*exfac; //10
	if (columns(s_mY)>ct_covid) mxtrel[0][ct_covid] = 20*exfac;	//20
	if (columns(s_mY)>ct_covid+1) mxtrel[0][ct_covid+1] = 20*exfac;	//20
	if (columns(s_mY)>ct_covid+2) mxtrel[0][ct_covid+2] = 10*exfac;	//10
	if (columns(s_mY)>ct_covid+3) mxtrel[0][ct_covid+3] = 1*exfac;
	//
	if (columns(s_mY)>ct_covid-2) mxtrel[1][ct_covid-2] = 1;
	if (columns(s_mY)>ct_covid) mxtrel[1][ct_covid-1] = 10;
	if (columns(s_mY)>ct_covid) mxtrel[1][ct_covid] = 20;
	if (columns(s_mY)>ct_covid+1) mxtrel[1][ct_covid+1] = 20;
	if (columns(s_mY)>ct_covid+2) mxtrel[1][ct_covid+2] = 10;
	...
	if (columns(s_mY)>ct_covid) mxtrel[2][ct_covid-2] = 2*exfac;  //2
	if (columns(s_mY)>ct_covid) mxtrel[2][ct_covid-1] = 4*exfac;  //4
	if (columns(s_mY)>ct_covid) mxtrel[2][ct_covid] = 4*exfac;	  //4
	if (columns(s_mY)>ct_covid+1) mxtrel[2][ct_covid+1] = 2*exfac;	//2
	if (columns(s_mY)>ct_covid+2) mxtrel[2][ct_covid+2] = 1*exfac;
	//
	if (columns(s_mY)>ct_covid-3) mxtrel[3][ct_covid-3] = 1;
	if (columns(s_mY)>ct_covid) mxtrel[3][ct_covid-2] = 1;
	if (columns(s_mY)>ct_covid) mxtrel[3][ct_covid-1] = 1;
...
SsfComponents1(const vPar)
{
	...
	mcmp |= msmo[cy+cy+(3*cy):cy+cy+(3*cy)+cy-2][]; // sampling error estimate 
...
ResidualsModeli(const fprint, const fgraph, const iMod, const vPar)
{
	...
	if (fgraph)
	{
		...
		SaveDrawWindow(sprint("Graphs_v2/ResidualsModel", iMod,"Covid.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
		...
		DrawAdjust(ADJ_AREAMATRIX, 4, 4);
		SaveDrawWindow(sprint("Graphs_v2/ResidualsModel", iMod,"Covid_v2a.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
		...
		DrawAdjust(ADJ_AREAMATRIX, 4, 4);
		SaveDrawWindow(sprint("Graphs_v2/ResidualsModel", iMod,"Covid_v2b.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
	}
	if (fprint)
...
ComponentsModel1(const fprint, const fgraph, const iMod, const vPar)
{
	...
	if (fgraph)
	{
		...
		DrawAdjust(ADJ_AREAMATRIX, 4, 5);
		SaveDrawWindow(sprint("Graphs_v2/ComponentsModel", iMod, "Covid.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
		/*s_igr=0;
		for (i=0; i<s_iN; i++)
		...
		SaveDrawWindow(sprint("Graphs_v2/TimevarylevelseModel", iMod, "Covid.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
		...
		DrawAdjust(ADJ_AREAMATRIX, 4, 1);
		SaveDrawWindow(sprint("Graphs_v2/TimevaryslopeseModel", iMod, "Covid.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();
		...
		for (i=0; i<s_iN-1; i++)
		{
			DrawTMatrix(s_igr++, mcmp[s_iN+s_iN+s_iN+s_iN+i][12:],  sprint("Sampling error ",  s_sNameY[i+1]), s_iFirstYear+3, s_iFirstQuarter, 4, 0, 5); // start in 2004 om rekening te houden met burnin periode kalman filter
		}
		DrawAdjust(ADJ_AREAMATRIX, 3, 1);
		SaveDrawWindow(sprint("Graphs_v2/SamplingErrorModel", iMod, "Covid.pdf"));
		ShowDrawWindow();
		CloseDrawWindow();*/
...
FullStudy(const iFirstYearFor, const iLastYearFor, const fprint, const fgraph, const fRealTimeParEst)
{
	...
	decl file_mse = fopen("table_mseCovid.tex", "w"), file_mae = fopen("table_maeCovid.tex", "w"), file_mape = fopen("table_mapeCovid.tex", "w"), file_pars = fopen("table_parsCovid.tex", "w");
	...
	if (fprint)
	{
		...
		printlatextable(file_mse, sprint("Forecasting performance"), iFirstYearFor, iLastYearFor, asformat, ones(4,1) ** range(1, 4)' ~ mMSE);
		printlatextable(file_mae, sprint("Forecasting performance"), iFirstYearFor, iLastYearFor, asformat, ones(4,1) ** range(1, 4)' ~ mMAE);
		printlatextable(file_mape, sprint("Forecasting performance"), iFirstYearFor, iLastYearFor, asformat, ones(4,1) ** range(1, 4)' ~ mMAPE);
	}
	...
	if (fgraph)
	{
		...
		for (j=0; j<=s_cMod-1; j++)
		{
			...
			SaveDrawWindow(sprint("Graphs_v2/Forecasts_model", j+1, "Covid.pdf"));
			CloseDrawWindow();
		}
		...
		for (j=0; j<=s_cMod-1; j++)
		{
			...
			SaveDrawWindow(sprint("Graphs_v2/Forecasts_uncertainty_model", j+1, "Covid.pdf"));
			CloseDrawWindow();
		}
		...
		for (j=0; j<=3; j++)
		{
			...
			SaveDrawWindow(sprint("Graphs_v2/Forecasts_scenario", j+1, "Covid.pdf"));
			CloseDrawWindow();
		}
		...
		for (j=0; j<=3; j++)
		{
			...
			SaveDrawWindow(sprint("Graphs_v2/Forecasts_uncertainty_scenario", j+1, "Covid.pdf"));
			CloseDrawWindow();
		}
		...
		SaveDrawWindow("Graphs_v2/Forecasts_model3_scen3Covid.pdf");
		CloseDrawWindow();
			
	}

	savemat("Output_v2/myforCovid.mat", myfor);
	savemat("Output_v2/myforseCovid.mat", myforse);
...
GetGraphs(const fgraph, const mx)
{
	if (fgraph)
	{
		...
		DrawAdjust(ADJ_MINMAX, 59.75, 67.75);
	 	DrawTMatrix(1, mx[0][:columns(mx)-2]~M_NAN, "LFS", s_iFirstYear, 1, 12, 0, 1);
		DrawAdjust(ADJ_MINMAX, 63.5, 72.5);
