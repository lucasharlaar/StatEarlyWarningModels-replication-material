...
ForecastModel1(const fprint, const fgraph, const fparest, const vParEst, const iscenario, const iyear)
{
	...
	if (iscenario == 2)
	{
		s_mY[0:1][ct-12:ct-1] = M_NAN;
		s_mY[2:][] = M_NAN;
	}
	if (iscenario == 3)
	{
		s_mY[0][ct-12:ct-1] = M_NAN;
		s_mY[1][ct-6:ct-1] = M_NAN;
		s_mY[2:][] = M_NAN;
	}
	if (iscenario == 4)
	{
		s_mY[0][ct-12:ct-1] = M_NAN;
		s_mY[1][ct-3:ct-1] = M_NAN;
		s_mY[2:][] = M_NAN;
	}
	if (iscenario == 5)
	{
...
FullStudy(const iFirstYearFor, const iLastYearFor, const fprint, const fgraph, const fRealTimeParEst)
{
	...
	for (iScenario=2; iScenario<=5; iScenario++)
	{
		for (s_iLevel=1, iMod=0; s_iLevel<2; s_iLevel++)
		{
			for (s_iSlope=0; s_iSlope<2; s_iSlope++)
			{
				s_iSeaso = s_iSlope;
				s_iIrreg = 4;
				iMod++;
	...
	savemat("Output_v3/myforFINRF-PRERFDC.mat", myfor);
	savemat("Output_v3/myforseFINRF-PRERFDC.mat", myforse);
...
main()
{
	...
	s_mY = mdata[<1,2,0,3>][2:435]; 				// select and order variables in y_t
	s_sNameY = {"FINRF","PRERFDC", "REGRF", "FINRFDC"};
	...
	//s_cMod = 6;
	//EstimateModel1(TRUE,FALSE, <>);
	//ComponentsModel1(TRUE, TRUE, s_cMod, s_vPar);
	//ResidualsModeli(TRUE, TRUE, s_cMod, s_vPar);
	...
	s_igr = 0;
	s_cMod=2;
	FullStudy(2011, 2022, TRUE, TRUE, TRUE);
	// last argument: re-estimate vpar for each forecast-year TRUE/FALSE
	
	/*decl iScenario=3, iFirstYearFor=2011, iLastYearFor=2022, vforstat, verr, vyobs, vyfor, vyforse, tyear, mPar=<>;

	// individual forecasts for 2022
	/*iYearFor = 2020;
	[vforstat, verr, vyobs, vyfor, vyforse] = ForecastModel1(TRUE, FALSE, TRUE, <>, iScenario, iYearFor);
	iYearFor = 2021;
	[vforstat, verr, vyobs, vyfor, vyforse] = ForecastModel1(TRUE, FALSE, TRUE, s_vPar, iScenario, iYearFor);
	iYearFor = 2022;
	[vforstat, verr, vyobs, vyfor, vyforse] = ForecastModel1(TRUE, FALSE, TRUE, s_vPar, iScenario, iYearFor);
	println("vyfor",vyfor);
	println("vyforse", vyforse);
	println("Monthly road fatalities 2022", exp(vyfor));
	println("Total road fatalities 2022", sumr(exp(vyfor)));*/

	for (tyear=iFirstYearFor, s_vPar = <>; tyear<=iLastYearFor; tyear++)
				{
					println("Year in this run: ", tyear);
					println("Forecast scenario in this run: ", iScenario);
					//println("MODEL ", iMod);
					println("Variance matrix settings in this run:");
					println("s_iLevel: ", s_iLevel); println("s_iSlope: ", s_iSlope); println("s_iSeaso: ", s_iSeaso); println("s_iIrreg: ", s_iIrreg);
					if (tyear == iLastYearFor) s_vPar = <>;
					[vforstat, verr, vyobs, vyfor, vyforse] = ForecastModel1(FALSE, FALSE, TRUE, s_vPar, iScenario, tyear);
					//mForStat ~= vforstat; verrtot ~= verr; vyobstot ~= vyobs, vyfortot ~= vyfor; vyforsetot ~= vyforse; 
					// save vPar of best model setting
					decl vPar = <>;
					vPar = exp(2.0*s_vPar[0]) | s_vPar[11:13] | s_vVarCorrir;
