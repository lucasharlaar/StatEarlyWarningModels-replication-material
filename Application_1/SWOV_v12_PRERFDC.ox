...
ForecastModel1(const fprint, const fgraph, const fparest, const vParEst, const iscenario, const iyear)
{
	...
	if (iscenario == 3)
	{
		s_mY[0][ct-6:ct-1] = M_NAN;
		s_mY[1:][] = M_NAN;
	}
	if (iscenario == 4)
	{
		s_mY[0][ct-3:ct-1] = M_NAN;
		s_mY[1:][] = M_NAN;
	}
	if (iscenario == 5)
	{
...
FullStudy(const iFirstYearFor, const iLastYearFor, const fprint, const fgraph, const fRealTimeParEst)
{
	...
	savemat("Output_v2/myforPRERFDC.mat", myfor);
	savemat("Output_v2/myforsePRERFDC.mat", myforse);
...
main()
{
	...
	s_mY = mdata[<2,1,0,3>][2:435]; 				// select and order variables in y_t
	s_sNameY = {"PRERFDC", "FINRF", "REGRF", "FINRFDC"};
